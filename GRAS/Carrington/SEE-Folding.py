...
path = "/home/anton/Desktop/triton_work/LET/LETAP910MeV/8mm/Res/"
#DataName = "Carrington SEP +2 Sigma 8mm Al"
DataName = "AP9 GTO 8mm"
...
plt.savefig("/home/anton/Desktop/TritonPlots/Carrington/SEE/LET-Rate" + DataName + " " + CrossectionName + ".pdf", format='pdf',
            bbox_inches="tight")
'''
...
SEERate = ufloat(SEERate, SEERateError)

print("The total SEE rate is:", SEERate, " s-1 bit-1 ")
print("or:", SEERate * 8e+6, " s-1 Mbyte-1 ")


fig, ax1 = plt.subplots(1)
plt.bar(SEEHist[:, lowerID], SEEHist[:, valueID], width=SEEHist[:, upperID] - SEEHist[:, lowerID], align='edge', alpha=0.3)
plt.errorbar(SEEHist[:, meanID], SEEHist[:, valueID], SEEHist[:, errorID], fmt=' ', capsize=5, elinewidth=1, capthick=1, label="SEE Histogram")
plt.plot([], [], label="SEE cross-section", color='C1')
plt.yscale("log")
plt.xscale("log")
plt.grid(which='both')
plt.title(DataName + "  &  " + CrossectionName + " \nTotal SEERate = " + str(SEERate) + " s-1 bit-1")
plt.xlabel("LET [MeV cm2 mg-1]")
ax1.legend(loc='center right')
...
plt.savefig("/home/anton/Desktop/TritonPlots/Carrington/SEE/SEE-Rate" + DataName + " " + CrossectionName + ".pdf", format='pdf', bbox_inches="tight")
