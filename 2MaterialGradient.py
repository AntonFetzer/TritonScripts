...
Path = "/home/anton/Desktop/triton_work/Gradient/2MaterialGradient/Pb-Al/csv/"

Shield = "Pb-Al"
MatA = "Lead"
MatB = "Aluminium"

ElecFileA = "gradient-pb-al-2e9electron.txt"
ElecFileB = "gradient-al-pb-2e9electron.txt"
ProtFileA = "gradient-pb-al-2e7proton.txt"
ProtFileB = "gradient-al-pb-2e7proton.txt"
...
plt.figure(2)
plt.plot(x + 1, TotalEdepA, '.', label=MatA + " on top of " + MatB + "  Min=" + str(round(np.min(TotalEdepA), 2)) + " krad at " + str(round(np.argmin(TotalEdepA)+1)) + " %")
plt.plot(x + 1, TotalEdepB, '.', label=MatB + " on top of " + MatA + "  Min=" + str(round(np.min(TotalEdepB), 2)) + " krad at " + str(round(np.argmin(TotalEdepB)+1)) + " %")
